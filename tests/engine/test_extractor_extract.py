...
async def test_extract_inferred_schema_validates_leniently():
    ...
    payload: dict = {}
    with (
        patch("crawl_tool.engine.extractor.infer_schema", AsyncMock(return_value=strict_schema)),
        patch("crawl_tool.engine.extractor.anthropic.AsyncAnthropic") as mock_client_cls,
    ):
        mock_client = AsyncMock()
        mock_client_cls.return_value = mock_client
        mock_client.messages.create = AsyncMock(return_value=_mock_response(json.dumps(payload)))
