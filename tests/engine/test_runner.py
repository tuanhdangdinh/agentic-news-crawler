...
async def test_execute_runs_agent_and_fills_injected_state(monkeypatch: pytest.MonkeyPatch):
    ...
    async def fake_run_agent(seed, config, state=None, *, proxy_rotator=None):
        state.pages.append(_page())
        state.finish_reason = "done"
        return state

    with patch("crawl_tool.engine.runner.run_agent", side_effect=fake_run_agent) as mock_run:
    ...
    assert mock_run.await_args.kwargs["proxy_rotator"] is None
...
async def test_execute_snapshots_agent_state_lists(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("crawl_tool.engine.runner.run_agent", side_effect=fake_run_agent):
        payload = await execute(request, state)
...
async def test_execute_excludes_html_and_raw_markdown(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("crawl_tool.engine.runner.fetch_page", AsyncMock(return_value=page)):
        payload = await execute(request, CrawlState())
...
@pytest.mark.asyncio
async def test_execute_passes_proxy_rotator_when_proxy_url_set(
    ...
    async def fake_run_agent(seed, config, state=None, *, proxy_rotator=None):
        captured["proxy_rotator"] = proxy_rotator
        state.pages.append(_page())
        state.finish_reason = "done"
    ...
    assert isinstance(captured.get("proxy_rotator"), ProxyRotator)


@pytest.mark.asyncio
async def test_execute_no_proxy_rotator_when_proxy_url_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PROXY_URL", raising=False)
    ...
    assert captured.get("proxy_rotator") is None
