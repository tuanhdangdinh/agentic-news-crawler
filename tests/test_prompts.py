...
def test_render_system_returns_nonempty_string():
    result = render("system.j2", **_SYSTEM_CTX)
    assert isinstance(result, str)
    assert len(result) > 0


def test_render_system_contains_goal():
    result = render("system.j2", **_SYSTEM_CTX)
    assert "collect economy news" in result


def test_render_user_turn_returns_nonempty_string():
    result = render("user_turn.j2", **_USER_CTX)
    assert isinstance(result, str)
    assert len(result) > 0


def test_render_user_turn_contains_url():
    result = render("user_turn.j2", **_USER_CTX)
    assert "https://cafef.vn" in result
