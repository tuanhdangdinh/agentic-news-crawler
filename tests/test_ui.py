...
from src.ui import _build_config, _parse_patterns, run_crawl
...
async def test_run_crawl_uses_direct_fetch_without_goal_or_extraction(tmp_path):
    ...
    with (
        patch("src.ui.fetch_page", AsyncMock(return_value=_page())) as mock_fetch,
        patch("src.ui._output_path", return_value=str(output_path)),
    ):
        status, payload, download = await run_crawl(
            "https://cafef.vn",
            "",
            "",
            "",
...
async def test_run_crawl_uses_agent_when_goal_is_present(tmp_path):
    ...
    with (
        patch("src.ui.run_agent", AsyncMock(return_value=state)) as mock_run_agent,
        patch("src.ui._output_path", return_value=str(output_path)),
    ):
        _, payload, download = await run_crawl(
            "https://cafef.vn",
