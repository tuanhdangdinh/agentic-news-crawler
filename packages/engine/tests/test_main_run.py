...
from crawl_engine.cli import build_parser, run
...
async def test_run_direct_fetch_writes_single_page_result():
    ...
    with (
        ...
        patch("crawl_engine.cli.fetch_page", AsyncMock(return_value=_page())) as mock_fetch,
        patch("crawl_engine.cli.write_results") as mock_write,
    ):
        await run(args)
...
async def test_run_agent_wires_week_3_config_flags():
    ...
    with (
        patch("crawl_engine.cli.configure_logging"),
        patch("crawl_engine.cli.run_agent", AsyncMock(return_value=state)) as mock_run_agent,
        patch("crawl_engine.cli.write_results"),
    ):
        await run(args)
...
async def test_run_max_depth_above_ceiling_skips_agent():
    ...
    with (
        patch("crawl_engine.cli.configure_logging"),
        patch("crawl_engine.cli.run_agent", AsyncMock()) as mock_run_agent,
        patch("crawl_engine.cli.write_results") as mock_write,
    ):
        await run(args)
