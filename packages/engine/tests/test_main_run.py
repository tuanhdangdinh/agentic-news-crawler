...
async def test_run_direct_fetch_writes_single_page_result(tmp_path):
    ...
    with (
        ...
        patch("crawl_engine.cli.execute", AsyncMock(return_value=payload)) as mock_execute,
    ):
        await run(args)
...
async def test_run_max_depth_above_ceiling_skips_agent():
    ...
    with (
        patch("crawl_engine.cli.configure_logging"),
        patch("crawl_engine.cli.execute", AsyncMock()) as mock_execute,
    ):
        await run(args)
...
async def test_run_dispatches_schema_file_operations_to_thread(tmp_path):
    ...
    with (
        patch("crawl_engine.cli.configure_logging"),
        patch("crawl_engine.cli.execute", AsyncMock(return_value=payload)),
        patch("crawl_engine.cli.asyncio.to_thread", wraps=asyncio.to_thread) as mock_to_thread,
    ):
        await run(args)
