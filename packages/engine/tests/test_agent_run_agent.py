...
async def test_run_agent_skips_failed_page_fetch(caplog):
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=failed)),
        patch("crawl_engine.agent.anthropic.AsyncAnthropic"),
        caplog.at_level(logging.WARNING, logger="crawl_engine.agent"),
    ):
...
async def test_run_agent_logs_fetch_at_info(caplog):
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=_page())),
        patch("crawl_engine.agent.anthropic.AsyncAnthropic") as mock_cls,
        caplog.at_level(logging.INFO, logger="crawl_engine.agent"),
    ):
        mock_client = AsyncMock()
        mock_cls.return_value = mock_client
        mock_client.messages.create = AsyncMock(return_value=_finish_response())
...
async def test_run_agent_logs_page_status_depth_and_fetch_time(caplog):
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=page)),
        patch("crawl_engine.agent.anthropic.AsyncAnthropic") as mock_cls,
        caplog.at_level(logging.INFO, logger="crawl_engine.agent"),
    ):
        mock_client = AsyncMock()
...
async def test_run_agent_date_filter_drops_out_of_range_article():
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=old_article)),
        patch("crawl_engine.agent.anthropic.AsyncAnthropic"),
    ):
...
async def test_run_agent_max_chars_truncates_markdown_sent_to_claude():
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=long_page)),
        patch("crawl_engine.agent.anthropic.AsyncAnthropic") as mock_cls,
    ):
        mock_client = AsyncMock()
        mock_cls.return_value = mock_client
...
async def test_run_agent_passes_css_selector_to_fetch_page():
    ...
    with (
        patch("crawl_engine.agent.fetch_page", AsyncMock(return_value=_page())) as mock_fetch,
        patch("crawl_engine.agent.anthropic.AsyncAnthropic") as mock_cls,
    ):
        mock_client = AsyncMock()
        mock_cls.return_value = mock_client
