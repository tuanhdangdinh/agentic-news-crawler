...
from crawl_engine.service import JOB_TTL_SECONDS, create_app
...
async def test_crawl_jobs_execute_serially():
    ...
    with patch("crawl_engine.service.execute", controlled_execute):
        async with _client(app) as client:
...
async def test_running_job_reports_live_pages_collected():
    ...
    with patch("crawl_engine.service.execute", update_state):
        async with _client(app) as client:
            created = (await client.post("/crawl", json={"seed_url": "https://a"})).json()
...
async def test_terminal_jobs_are_purged_after_ttl(monkeypatch):
    now = [100.0]
    monkeypatch.setattr("crawl_engine.service._monotonic", lambda: now[0])
    app = create_app()
