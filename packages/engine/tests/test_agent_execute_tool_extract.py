...
import pytest
from crawl_engine.agent import AgentConfig, CrawlState, _execute_tool
...
async def test_extract_stores_result_on_current_page_metadata():
    ...
    with patch("crawl_engine.agent.extractor_extract", AsyncMock(return_value={"title": "CafeF"})):
        result = await _execute_tool(
            "extract",
            {"prompt": "extract title"},
            CrawlState(),
...
async def test_extract_infers_schema_once_when_config_schema_missing():
    ...
    schema = {"type": "object", "properties": {"title": {"type": "string"}}}
    with (
        patch("crawl_engine.agent.infer_schema", AsyncMock(return_value=schema)) as mock_infer,
        patch("crawl_engine.agent.extractor_extract", AsyncMock(return_value={"title": "CafeF"})) as mock_extract,
    ):
        result = await _execute_tool(
            "extract",
            {"prompt": "extract title"},
    ...
    mock_extract.assert_called_once_with(page, "extract title", schema, client=None, lenient=True)
...
async def test_extract_uses_registered_schema_before_inference():
    ...
    with (
        patch("crawl_engine.agent.infer_schema", AsyncMock()) as mock_infer,
        patch(
            "crawl_engine.agent.extractor_extract",
    ...
    mock_extract.assert_called_once_with(
        page, prompt, config.extract_schema, client=None, lenient=False
    )
    assert config.extract_schema_inferred is False
...
async def test_extract_stores_error_on_current_page_metadata():
    page = _page()
    schema = {"type": "object", "properties": {"title": {"type": "string"}}}
    with patch("crawl_engine.agent.extractor_extract", AsyncMock(return_value={"error": "bad json"})):
        result = await _execute_tool(
            "extract",
            {"prompt": "extract title"},
            CrawlState(),
